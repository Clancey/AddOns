...
local UnitName, CastingInfo, ChannelInfo = UnitName, CastingInfo, ChannelInfo
...
function CastBarTemplate:UNIT_SPELLCAST_START(event, unit, guid, spellID)
	...
	if self.casting then
		spell, displayName, icon, startTime, endTime, isTradeSkill, castID, notInterruptible = CastingInfo(unit)
	else -- self.channeling
		spell, displayName, icon, startTime, endTime, isTradeSkill, notInterruptible = ChannelInfo(unit)
...
function CastBarTemplate:UNIT_SPELLCAST_DELAYED(event, unit)
	...
	if self.casting then
		spell, displayName, icon, startTime, endTime = CastingInfo(unit)
	else
		spell, displayName, icon, startTime, endTime = ChannelInfo(unit)
	end
...
function CastBarTemplate:UpdateUnit()
	if CastingInfo(self.unit) then
		self:UNIT_SPELLCAST_START("UNIT_SPELLCAST_START", self.unit)
	elseif ChannelInfo(self.unit) then
